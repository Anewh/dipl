    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div v-if="status === 'show'">
                ...
                <p class="card-text">
                <p><a :href='link'
                       class="link-success link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover">{{ link_name }}</a>
                </p>
                </p>
            </div>
            <div v-else-if="status === 'edit'">
                <div class="d-flex flex-row align-items-center">
                    <SelectField :modelValue="type" @update:modelValue="$emit('update:type', $event)" label="Тип"
                                 placeholder='текст' :options="{
                                     text: 'текст',
                                     link: 'ссылка'
                                 }" />
...
export default {
    ...
    methods: {
        save() {
            ...
            this.status = 'save';
            fetch(`/project/${this.projectId}/field/${this.id}/edit`, {
                method: 'POST',
        ...
        deleteCard() {
            const body = structuredClone(this.$props);
            fetch(`/project/${this.projectId}/field/${this.id}/delete`, {
                method: 'POST',
                body: JSON.stringify(body),
                headers: { 'Content-Type': 'application/json' }
            })
